...
function ProjectCard({ project, priority }: { project: Project; priority?: boolean }) {
  ...
  return (
    <Link
      ...
      className="group cursor-pointer relative overflow-hidden rounded-3xl border transition-all duration-500 hover:-translate-y-2 flex flex-col min-h-[220px] sm:min-h-[300px] md:min-h-[380px] bg-white border-black/5 shadow-sm dark:bg-brand-darkSurface dark:border-white/5 dark:shadow-none"
    >
      <div className="w-full h-full relative overflow-hidden">
        <Image
          ...
          onError={() => {
            setImgSrc('https://placehold.co/1200x600/16171A/8A8F98?text=Image+Load+Failed');
          }}
        />
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-black/45 to-transparent p-8 flex flex-col justify-end">
          {project.techStack && project.techStack.length > 0 && (
            <div className="inline-flex items-center justify-center bg-black/65 backdrop-blur-md border border-white/10 rounded-full px-4 py-1 text-xs font-mono font-bold tracking-widest text-brand-lime uppercase w-fit mb-4">
              {project.techStack[0]}
            </div>
          )}
